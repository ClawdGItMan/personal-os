...
export function ProviderConnectionRow({
  ...
  const disconnectFn = provider === "whoop" ? disconnectWhoop : disconnectGoogle;
  const syncFn = provider === "whoop" ? syncWhoopNow : syncGoogleNow;
  ...
  useEffect(() => {
    ...
    if (params.get("connected") !== provider || triggered.current) return;
  ...
  }, [router, provider, syncFn]);

  const meta = status ? META[status] : null;
  const isConnected = Boolean(status);

  function handleDisconnect() {
    if (!window.confirm(`Disconnect ${label}? It will stop syncing.`)) return;
  ...
  return (
    <div className="grid grid-cols-[28px_1fr_auto] gap-3 items-center py-2.5 border-t border-[color:var(--os-line-1)] first:border-t-0">
      ...
      <div className="min-w-0">
        <div className="truncate text-sm text-[color:var(--os-fg-1)]">{label}</div>
